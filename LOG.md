...
- 分頁改名：「賭注」→ **what if**、「腳本」→ **遇到**。頁面眉標與 Markdown 匯出的段落標題都引用同一個常數，不會各叫各的。
- 「匯出 Markdown」→「**匯出**」。展開後的回饋仍寫著「可貼進 Obsidian」，格式資訊留在那裡就夠了。
- 外觀切換改成單一字元：◐ 跟隨系統／○ 淺／● 深。
- 實測 393px：六個按鈕（今天、模式、what if、遇到、匯出、◐）全在同一行，內容寬 349px 用掉 274px。

**決策與備註**

- **改名的理由不只是排版。** 「what if」正是那個分頁自己在說的話（「如果我說了，會怎樣？」），「遇到」正是每一組腳本的組頭（「遇到 · 場合」）。名字往內容靠，不是為了短而短。
- **「what if」其實比「賭注」寬**（47px vs 30px），單看它是讓換行更糟。是品牌獨立成行釋出的空間吸收掉的。
