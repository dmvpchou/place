...
## 2026-07-31 · 第六輪：⋯ 選單、完整紀錄、頁尾

**完成事項**

- 側欄底部收斂成一個「⋯」，裡面是**完整紀錄／匯出／備份**。原本三個常駐項目（匯出、備份、外觀）現在只剩一個。
- 外觀切換移到品牌那一行的右側，兩者都放大：品牌 17px → 20px（新的 `.t-brand`），圖示 13px → 16px。
- **完整紀錄**：不在分頁列上的回顧視圖，時間軸不設上限（「7 筆 · 全部」）。工作台因此收了一個 `scope` 參數。
- 工作台的時間軸由「最近三週」改成**最近 14 天**，與條狀圖同一個 `BAR_DAYS`，兩邊看到的範圍一致。
- 工作台變成三段式響應：≥1200px 三欄；<1200px 收掉右欄（模式本來就有自己的分頁）；<900px 一次只顯示列表或詳情，加一個「← 回到列表」。
- 「匯出」的回饋不再提 Obsidian，只說「已複製 N 天記錄」。
- 頁尾：`未寄 2026 · 咖啡宇宙的一部份`。
- 實測 393px：品牌＋外觀一行、四個分頁＋⋯ 一行，共兩行；完整紀錄在窄螢幕走完 列表 → 詳情 → 回到列表。

**決策與備註**

- **「完整紀錄」不是第五個分頁。** 四個分頁是同一個整理動作的四個階段，塞第五個進去會破壞那個結構。它是從「⋯」進來的視圖，`Tab` 型別多一個 `'records'`，但不出現在分頁列上。
- **工作台重用而不是另寫一個列表。** 完整紀錄與寬螢幕的「模式」是同一個元件，差別只有 `scope='all'`。
- **⋯ 選單吃掉了 BackupPanel。** 匯出與備份現在同屬一個面板的不同階段，狀態機併在 `MoreMenu` 裡，側欄回到只負責導覽。
- **README 的分頁名同步校正**：第四輪改名時只動了程式，README 的表格還停在「賭注／腳本」。

---

## 2026-07-31 · 第五輪：備份與還原

起因：沒有帳號、沒有同步，所有東西只活在一個瀏覽器的 localStorage。使用者清一次瀏覽資料、換一支手機、系統回收儲存空間，記錄就全沒了。原本的「匯出」是 Markdown，好讀但救不回來。

**完成事項**

- `lib/backup.ts`：`toBackup` 產生帶格式標記的 JSON、`parseBackup` 讀回來、`downloadBackup` 用 Blob + `<a download>` 存到本機。全程不經過任何伺服器。
- 側欄新增「備份」，點開後「存成檔案」與「從備份還原」並列。
- 還原是全站唯一會蓋掉既有記錄的動作，所以先顯示「這份備份有 N 天記錄、M 個賭注」與「換上去之後，現在這台裝置上的內容會被它取代」，再問一次。
- 6 個單元測試（round-trip、筆數、壞檔案、壞資料、空狀態、檔名），共 29 個。
- 實測：注入合成備份檔走完整條還原路徑，localStorage 由 2 筆變 3 筆、賭注 1 個變 2 個，畫面即時更新。

**決策與備註**

- **「還原」不能藏在「存檔」後面。** 第一版把還原入口放在存檔完成的面板裡，做出來才發現順序是錯的：真正需要還原的人，資料通常剛好已經沒了，卻得先備份一份空的才看得到還原。改成點開就兩個並列。
- **還原後強制 reload。** 各畫面的狀態是掛載時算出來的（「今天」的四種狀態、逐題流程的進度、工作台選了哪一天），整份資料被抽換之後那些值全是舊的——實測前先看到「今天」還停在還原前的「好，今天就這樣。」。換完顯示「換好了。」800ms 後重載，讓每個畫面從新資料重新算。
- **驗證沿用 `persist` 的 `migrate`。** 備份檔跟 localStorage 一樣可能是舊版或壞的，同一套清洗規則就夠了，不另寫一份。
- **備份檔沒有加密**，README 據實寫明——它跟記錄本身一樣私密，放哪裡由使用者決定。
- **只多一個頂層按鈕。** 側欄底部已經有「匯出」與外觀圖示，窄螢幕橫列排得下是有上限的，所以還原收在面板裡而不是再開一個按鈕。
